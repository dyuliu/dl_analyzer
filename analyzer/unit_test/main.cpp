...
DEFINE_string(action, "batch", "recorder, stat, distance, batch");
DEFINE_string(src, "records", "the specify file/folder path");
...
static inline void analyzer_batch_distance(std::vector<Infos> &batch_infos) {
	...
	for (int idx = 0; idx < last_batch; idx++) {
		...
		batch_infos[idx].print_distance_info(Infos::TYPE_CONTENT::GRAD);
		auto data = batch_infos[idx].get_content_data(Infos::TYPE_CONTENT::GRAD, "fc1000");
		analyzer::filestream::output_vector_to_file(data, batch_infos[idx].get().filename()+".txt");
	}
	auto data = batch_infos[last_batch].get_content_data(Infos::TYPE_CONTENT::GRAD, "fc1000");
	analyzer::filestream::output_vector_to_file(data, batch_infos[last_batch].get().filename() + ".txt");
	emath::normalization(data, emath::norm_method::MINMAX);
	analyzer::filestream::output_vector_to_file(data, batch_infos[last_batch].get().filename() + "_norm.txt");
}
...
void analyzer_tools() {
	...
	if (!analyzer::filesystem::exist(FLAGS_src.c_str())) {
		std::cout << "Error: Missing folder path!" << std::endl;
